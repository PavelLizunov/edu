...
### `<CodeBlock>`
```mdx
<CodeBlock lang="yaml" label="filename.yaml">
content here
</CodeBlock>
```
- `lang` — yaml, bash, sh, json, dockerfile, hcl/terraform, javascript, typescript, python, sql, nginx, ini, toml, diff, html, css, text
- `label` — опционально. Имя файла или короткий заголовок.
